...
// REMOVED: Don't set window.type here - it causes SSR issues
...
const MapComponent = ({ textMode, editDetails, features, setFeatures, setSelectionHandlers }) => {
    ...
    // Set up Leaflet icon configuration only on client side
    // React.useEffect(() => {
    ...
    // const EditControlWithoutSSR = dynamic(() => import("react-leaflet-draw").then(mod => mod.EditControl), { ssr: false });
    ...
    // Expose fetchSegmentationData to window object - only on client side
    React.useEffect(() => {
        // if (typeof window !== 'undefined') {
            // Use setTimeout to ensure DOM is ready
            // setTimeout(() => {
                const maptest = document.querySelector('#map-container');
                if (maptest) {
                    maptest.fetchSegmentationData = fetchSegmentationData;
                }
            // }, 0);
        // }
